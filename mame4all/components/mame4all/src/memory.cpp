											   ...
											   mem_write_handler handler, int install_at_beginning)
{
	...
	if (writeport[cpu] == 0)
	{
		writeport[cpu] = (IOWritePort *) (struct IOWritePort *)malloc(writeport_size[cpu]);
	}
	else
	{
		writeport[cpu] = (IOWritePort *) (struct IOWritePort *)realloc(writeport[cpu], writeport_size[cpu]);
	}
