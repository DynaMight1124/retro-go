...
The reference measurement is Celeste using the same play route for each build. `baseline.txt` records the unedited CrokPocket port. Performance results below were collected from the corresponding hardware logs; the current `log.txt` records the final CrokPocket lifecycle test.

| Target and scene | Unedited port | Optimized port | Final observation |
| :--- | :--- | :--- | :--- |
| CrokPocket (ESP32-S3), typical rooms | 11-13 FPS | 28-30 FPS | Lua 8-9 ms, audio mix 19-20 ms |
| CrokPocket (ESP32-S3), script-heavy room | Not reached in the baseline run | 20-22 FPS | Lua about 20 ms, audio mix about 22 ms |
...
```text
python rg_tool.py --target crokpocket build fake-08
```
