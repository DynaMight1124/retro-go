...
{
	static Bit32s *buffer = NULL;
	if (buffer == NULL)
		buffer = (Bit32s *)rg_alloc(512 * 2 * sizeof(Bit32s), MEM_SLOW);
...
void SD_ClearSoundCache(void)
{
	if (audioMutex) SDL_LockMutex(audioMutex);
	for(unsigned int i = 0; i < SoundInfo.GetNumSounds(); i++)
	{
		SoundInfo.GetSound(i).ClearDigitalData();
	}
...
{
	return (Sint64)((MacSoundData*)ops->hidden.unknown.data1)->size + 44; // 40 byte WAV_HEADER + 4 byte ssize
}
*/
...
#endif
{
	...
	switch(relative)
	{
		...
		case RW_SEEK_CUR:
			...
			break;
		case RW_SEEK_END:
			curpos = ((MacSoundData*)ops->hidden.unknown.data1)->size+44+pos;
			break;
	}
	return (int)curpos;
}
...
#endif
{
	...
	DWORD ssize = (DWORD)(((MacSoundData*)ops->hidden.unknown.data1)->size);
	...
	return nmem; // Return nmem to indicate success
}
...
Mix_Chunk* SD_PrepareSound(int which)
{
	...
	// Bypass FMemLump/FString entirely to save 50% peak memory overhead!
	// We read directly into a PSRAM buffer.
	uint8_t *raw_buf = (uint8_t *)heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
	if (!raw_buf || ((uint32_t)raw_buf < 0x3f800000 && size > 4096)) {
		SD_ClearSoundCache();
		if (raw_buf && (uint32_t)raw_buf < 0x3f800000) { free(raw_buf); raw_buf = NULL; }
		if (!raw_buf) raw_buf = (uint8_t *)heap_caps_malloc(size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
	}

	if (!raw_buf) {
		RG_LOGE("SD_PrepareSound: Failed to allocate %d bytes for sound lump %d\n", size, which);
		return NULL; // Graceful degradation (silent gunshot) instead of OOM panic
	}

	FWadLump lumpr = Wads.OpenLumpNum(which);
	lumpr.Read(raw_buf, size);

	if(BigShort(*(WORD*)raw_buf) == 1 && size > 0x2A)
	{
		...
		memcpy(((MacSoundData*)ops->hidden.unknown.data1)->data, raw_buf+0x2A, size-0x2A);
	...
	Mix_Chunk *result = Mix_LoadWAV_RW(SDL_RWFromMem(raw_buf, size), 1);
	free(raw_buf);
	return result;
}
...
int SD_PlayDigitized(const SoundData &which,int leftpos,int rightpos,SoundChannel chan)
{
	...
	// Check free PSRAM and clear cache if low to prevent OOM
	// Increased threshold to 1MB to give the engine more breathing room.
...
//
    ...
    int ret_chan = Mix_PlayChannel(channel, sample, 0);
    // RG_LOGW("SD_PlayDigitized: samples=%d, target_chan=%d, ret_chan=%d\n", (int)sample->alen, channel, ret_chan);
	if(ret_chan == -1)
	{
...
{
	static int lastMusicMode = -1;
	if (lastMusicMode != MusicMode) {
		// printf("SDL_MixEmulators: MusicMode changed to %d\n", MusicMode);
		lastMusicMode = MusicMode;
	}
	...
	if(len > stream_len)
	{
		...
		if(AudioCVTStereo.needed && AudioCVTStereo.len_mult > AudioCVTStereo.len_ratio)
			...
			stream_len = len;
		stream = (Uint8*)heap_caps_realloc(stream, stream_len, MALLOC_CAP_SPIRAM);
		if (!stream) {
			stream_len = 0;
			return;
		}
...
SD_Startup(void)
{
    ...
    param_samplerate = 11025; // Force ESP32 compatible sample rate, overriding config.cfg
...
{
	if (sound && strcmp(sound, "None") == 0)
	{
//
		return 0;
	}

	SoundIndex sindex = SoundInfo.FindSound(sound);
	if (sindex == 0 && sound && sound[0] != '\0')
	{
//
	}

	// name=%s chan=%d\n", sound ? sound : "(null)", (int)chan);
	return SD_PlaySound(sindex, chan);
}

int SD_PlaySound(SoundIndex sindex, SoundChannel chan)
{
	bool            ispos;
	int             lp,rp;
...
SD_ContinueMusic(const char* chunk, int startoffs)
{
	...
	if (MusicMode != smm_Off)
	{
		...
		else
		{
			...
			Mix_SetMusicPCMPosition((double)startoffs);
		}
	}
}
