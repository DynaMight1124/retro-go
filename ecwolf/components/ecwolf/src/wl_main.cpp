...
int      viewheight;
int      statusbarx;
int      statusbary1, statusbary2; short    centerx;
short    centerxwide;
...
static void InitGame()
{
	...
	I_InitGraphics(); // Allocate PSRAM arrays before table generation
...
void I_FatalError (const char *format, ...)
{
    ...
    rg_system_panic("ECWolf Fatal", error.GetChars());
...
void I_Error(const char* format, ...)
{
    ...
    rg_system_panic("ECWolf Error", error.GetChars());
    while(1); // Should not reach here
}
...
static const char* CheckParameters(int argc, char *argv[], TArray<FString> &files)
{
	...
	for(int i = 1; i < argc; i++)
	{
		...
		else IFARG("--skill")
		{
			...
			else param_difficulty = atoi(argv[i])-1; // 1-based indexing
		}
		else IFARG("--nowait")
			param_nowait = true;
		else IFARG("--tedlevel")
...
static void ScannerMessageHandler(Scanner::MessageLevel level, const char *error, va_list list)
{
	...
	if(level == Scanner::ERROR)
		I_Error("%s", errorMessage.GetChars());
	else
...
int WL_Main (int argc, char *argv[])
{
	...
	catch(...) // Catch all to avoid stub abort
	{
