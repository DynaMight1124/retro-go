...
void FizzleFadeStart()
{
	screen->Lock(false);
#ifdef ESP_PLATFORM
	fizzleSurface = (byte *)heap_caps_malloc(SCREENHEIGHT*SCREENPITCH, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
	...
	if (fizzleSurface) {
		...
		RG_LOGE("FizzleFadeStart: Failed to allocate %d bytes for fizzle surface", SCREENHEIGHT*SCREENPITCH);
	}
	screen->Unlock();
}
	...
	unsigned width, unsigned height, unsigned frames, bool abortable)
{
	...
	if (!fizzleSurface) return false;

	pixperframe = width * height / (frames / 4 + 1); // Speed up the fade significantly
...
#ifdef ESP_PLATFORM
	byte *srcptr = (byte *)heap_caps_malloc(SCREENHEIGHT*SCREENPITCH, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
	byte *srcptr = new byte[SCREENHEIGHT*SCREENPITCH];
#endif

	if (!srcptr) {
		RG_LOGE("FizzleFade: Failed to allocate %d bytes for srcptr", SCREENHEIGHT*SCREENPITCH);
		screen->Unlock();
#ifdef ESP_PLATFORM
		free(fizzleSurface);
#else
...
#endif
		fizzleSurface = NULL;
		return false; // Skip the fade effect gracefully
	}
	...
	{
		IN_ProcessEvents();
		rg_task_delay(1); // Give other tasks time
