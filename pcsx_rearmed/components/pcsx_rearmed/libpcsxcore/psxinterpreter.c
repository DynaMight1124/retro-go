...
// Subsets
static void (INT_ATTR *psxBSC[64])(psxRegisters *regs_, u32 code) CORE_DATA_ATTR;
static void (INT_ATTR *psxSPC[64])(psxRegisters *regs_, u32 code) CORE_DATA_ATTR;
...
static inline void addCycle(psxRegisters *regs)
{
	...
	// 8: 8x Speedup (Aggressive).
	// 12+: Max Speed (Likely to cause logic timing issues).
	static int skip = 0;
	if (++skip < 4) return;
	skip = 0;
