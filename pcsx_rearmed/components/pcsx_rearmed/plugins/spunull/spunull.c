...
unsigned short *regArea;                        // psx buffer
unsigned short *spuMem;                         // spu buffer
...
void CALLBACK builtin_SPUwriteRegister(unsigned long reg, unsigned short val, unsigned int cycles)
{
...
void CALLBACK builtin_SPUwriteDMAMem(unsigned short * pusPSXMem,int iSize,unsigned int cycles)
{
 int i;
 for(i=0;i<iSize;i++)
  {
...
void CALLBACK builtin_SPUreadDMAMem(unsigned short * pusPSXMem,int iSize,unsigned int cycles)
{
 int i;
 for(i=0;i<iSize;i++)
  {
...
void CALLBACK builtin_SPUplayADPCMchannel(xa_decode_t *xap, unsigned int cycles, int is_start)
{
}

void CALLBACK builtin_SPUsetCDvol(unsigned char ll, unsigned char lr,
...
long CALLBACK builtin_SPUinit(void)
{
 regArea = heap_caps_calloc(10000, sizeof(unsigned short), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
 spuMem  = heap_caps_calloc(256*1024, sizeof(unsigned short), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
 spuMemC = (unsigned char *)spuMem;
 return 0;
}

long CALLBACK builtin_SPUshutdown(void)
{
 if(regArea) { free(regArea); regArea = NULL; }
 if(spuMem)  { free(spuMem);  spuMem  = NULL; }
 return 0;
}
...
long CALLBACK builtin_SPUopen(void)
{
...
long CALLBACK builtin_SPUclose(void)
{
 if(!bSPUIsOpen) return 0;
 bSPUIsOpen=0;
 return 0;
...
void builtin_SPUasync(unsigned int cycle, unsigned int flags)
{
}

int builtin_SPUplayCDDAchannel(short *pcm, int nbytes, unsigned int cycle, int is_start)
{
 return -1;
}
...
void CALLBACK builtin_SPUregisterCallback(void (CALLBACK *callback)(void))
{
 ...
 for(i=0;i<0x100;i++)
  {
   if(i!=H_SPUon1-0xc00 && i!=H_SPUon2-0xc00)
    builtin_SPUwriteRegister(0x1f801c00+i*2,regArea[i],cycles);
  }
 builtin_SPUwriteRegister(H_SPUon1,regArea[(H_SPUon1-0xc00)/2],cycles);
 builtin_SPUwriteRegister(H_SPUon2,regArea[(H_SPUon2-0xc00)/2],cycles);
...
void CALLBACK builtin_SPUregisterScheduleCb(void (CALLBACK *callback)(unsigned int))
{
}
