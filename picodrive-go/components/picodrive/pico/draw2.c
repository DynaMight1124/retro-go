...
unsigned char *PicoDraw2FB_ = NULL;

static u32 HighCache2A[2*41*(TILE_ROWS+1)+1+1]; // caches for high layers
static u32 HighCache2B[2*41*(TILE_ROWS+1)+1+1];

unsigned short *PicoCramHigh = NULL; // pointer to CRAM buff (0x40 shorts), converted to native device color (works only with 16bit for now)
...
PICO_INTERNAL void PicoFrameFullTile(void)
{
	pprof_start(draw);
