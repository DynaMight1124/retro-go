...
extern u32 PicoRead8_mcd_io(u32 a);
extern u32 PicoRead16_mcd_io(u32 a);
extern void PicoWrite8_mcd_io(u32 a, u32 d);
extern void PicoWrite16_mcd_io(u32 a, u32 d);
...
static u8 dummy_fetch_bank[1 << (24 - FAMEC_FETCHBITS)];
...
#ifdef EMU_F68K
  // Automatically update FAME fetchmap for all readable memory regions
  if (map == m68k_read8_map || map == m68k_read16_map || 
      map == s68k_read8_map || map == s68k_read16_map) {
    int is_sub = (map == s68k_read8_map || map == s68k_read16_map);
    ...
    if (!(is_func & 1)) {
      // Normal memory
      base = (uptr)func_or_mh - (i << shiftout);
    } else {
      // Unmapped/Function - use dummy bank
      base = (uptr)dummy_fetch_bank - (i << shiftout);
    }
...
u32 PicoRead8_io(u32 a)
{
  ...
  if (PicoIn.AHW & PAHW_MCD) {
    if ((a & 0xffc0) == 0x2000) return PicoRead8_mcd_io(a);
  }
...
u32 PicoRead16_io(u32 a)
{
  ...
  if (PicoIn.AHW & PAHW_MCD) {
    if ((a & 0xffc0) == 0x2000) return PicoRead16_mcd_io(a);
  }
...
{
  if (PicoIn.AHW & PAHW_MCD) {
    if ((a & 0xffc0) == 0x2000) { PicoWrite8_mcd_io(a, d); return; }
  }
...
{
  if (PicoIn.AHW & PAHW_MCD) {
    if ((a & 0xffc0) == 0x2000) { PicoWrite16_mcd_io(a, d); return; }
  }
...
void PicoMemInit(void)
{
#ifdef EMU_F68K
  int i;
  u16 *p = (u16 *)dummy_fetch_bank;
  for (i = 0; i < sizeof(dummy_fetch_bank) / 2; i++)
    p[i] = 0x4afc; // ILLEGAL
#endif
}
