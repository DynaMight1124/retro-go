...
#define SekInterrupt(irq) SekInterruptM68k(irq)
...
extern unsigned char (*HighLnSpr)[32];
extern unsigned char *HighColBase;
extern int HighColIncrement;
extern void *DrawLineDestBase;
extern int DrawLineDestIncrement;
extern u32 *VdpSATCache_ptr;
#define VdpSATCache VdpSATCache_ptr

// famec / musashi / cyclone
void SekRun(int cycles);
void SekRunS68k(unsigned int to);

// draw2.c
void PicoDraw2SetOutBuf(void *dest, int incr);
void PicoDraw2Init(void);
void PicoFrameFullTile(void);
...
// memory.c
PICO_INTERNAL void PicoMemInit(void);
...
extern struct PicoMemory *PicoMem_ptr;
...
// Unconditionally declare these to satisfy draw.c and others when PICO_32X is off
void PicoDrawSetOutFormat32x(pdso_t which, int use_32x_line_mode);
void PicoDrawSetOutBuf32X(void *dest, int increment);
u32  PicoRead8_32x(u32 a);
u32  PicoRead16_32x(u32 a);
