...
{
	char *current_file = malloc(256);
	char *cue_base = malloc(256);
	char *buff = malloc(256);
	char buff2[32], ext[4], *p;
	char *current_filep;
	...
	if (!current_file || !cue_base || !buff) goto out_free;

	if (fname == NULL || (fname_len = strlen(fname)) == 0)
		goto out_free;

	ret = get_ext(fname, ext, cue_base, 256 - 4);
	...
	snprintf(current_file, 256, "%s", fname);
	...
	current_filep_size = 256 - (current_filep - current_file);

	// the basename of cuefile, no path
	snprintf(cue_base, 256, "%s", current_filep);
	p = strrchr(cue_base, '.');
	if (p)	p[1] = '\0';
