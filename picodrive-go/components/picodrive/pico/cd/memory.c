...
void m68k_comm_check(u32 a)
{
  ...
  if (!(PicoIn.opt & POPT_DIS_IDLE_DET) && Pico_mcd->m.m68k_poll_cnt >= POLL_LIMIT) {
    Pico_mcd->m.state_flags |= PCD_ST_M68K_POLL;
    SekEndRun(8);
  }
}
...
u32 m68k_reg_read16(u32 a)
{
  ...
  switch (a) {
    ...
    case 2:
      ...
      // Hardware quirk: If Sub-CPU is in reset, Word RAM is always returned to Main CPU (RET=1).
      // We check busreq bit 0 (Reset bit): 0 = Reset, 1 = Run.
...
void m68k_reg_write8(u32 a, u32 d)
{
  ...
  switch (a) {
    ...
    case 1:
      ...
      // Immediate sync to ensure both CPUs are at the same time point
      pcd_sync_s68k(SekCyclesDone(), 0);
      ...
      if (!(d & 1)) {
        ...
        if (Pico_mcd->m.state_flags & PCD_ST_S68K_RST) {
          Pico_mcd->m.state_flags &= ~(PCD_ST_S68K_RST|PCD_ST_S68K_POLL|PCD_ST_S68K_SLEEP);
          elprintf(EL_CDREGS, "m68k: releasing s68k from reset");
          SekResetS68k();
          SekCycleCntS68k += 40;
        }
      }
