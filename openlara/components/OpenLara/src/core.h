...
#define H_CORE
    ...
    #define COLOR_16
    #include <retro-go.h>
    #define OL_ALLOC(size) rg_alloc(size, MEM_SLOW)
    #define OL_FREE(p)     rg_free(p)
    #define OL_ALLOC_NP(size) rg_alloc(size, MEM_SLOW | MEM_NOPANIC)
    #define OL_REALLOC(p, size) realloc(p, size)
    #define OL_REALLOC_NP(p, size) realloc(p, size)
    #define INV_GAMEPAD_ONLY
    #define INV_STEREO
#else
    #define OL_ALLOC(size) malloc(size)
    #define OL_FREE(p)     free(p)
    #define OL_ALLOC_NP(size) malloc(size)
    #define OL_REALLOC(p, size) realloc(p, size)
    #define OL_REALLOC_NP(p, size) realloc(p, size)
#endif
...
#define USE_CUBEMAP_MIPS
    ...
    #if !defined(_OS_GCW0) && !defined(__RETROGO__)
        #define MERGE_MODELS
    #endif
...
namespace Core {
    ...
    */
    ...
    // init settings
        ...
        settings.detail.setFilter   (Core::Settings::LOW);
    ...
    // player 1
        {
            ...
            ctrl.keys[ cJump      ] = KeySet( ikAlt,    jkB      ); // B for Jump
            ctrl.keys[ cWalk      ] = KeySet( ikShift,  jkRB     ); // R1 for Walk
            ctrl.keys[ cAction    ] = KeySet( ikCtrl,   jkA      ); // A for Action/Fire
            ctrl.keys[ cWeapon    ] = KeySet( ikSpace,  jkY      ); // Y or START for Weapon (mapped in main.cpp)
            ctrl.keys[ cLook      ] = KeySet( ikC,      jkLB     ); // L1 for Look
            ctrl.keys[ cDuck      ] = KeySet( ikZ,      jkX      ); // X or SELECT for Duck (mapped in main.cpp)
            ctrl.keys[ cDash      ] = KeySet( ikX,      jkRT     ); // R2 for Dash
            ctrl.keys[ cRoll      ] = KeySet( ikA,      jkLT     ); // L2 for Roll
            ctrl.keys[ cInventory ] = KeySet( ikEscape, jkSelect ); // SELECT/MENU for Inventory (mapped in main.cpp)
            ctrl.keys[ cStart     ] = KeySet( ikEnter,  jkStart  ); // START for Start/Pause (overridden in main.cpp mapping)
        }
    ...
    // player 2
        {
            ...
            ctrl.keys[ cJump      ] = KeySet( ikNone,   jkB      );
            ...
            ctrl.keys[ cDuck      ] = KeySet( ikNone,   jkX      );
            ctrl.keys[ cDash      ] = KeySet( ikNone,   jkRT     );
            ctrl.keys[ cRoll      ] = KeySet( ikNone,   jkLT     );
