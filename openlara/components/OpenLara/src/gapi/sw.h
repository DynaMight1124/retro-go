...
// Texture
    struct Texture {
        ...
        Texture(int width, int height, int depth, TexFormat fmt, uint32 opt) : memory(0), width(width), height(height), origWidth(width), origHeight(height), fmt(fmt), opt(opt) {}
        ...
        void init(void *data) {
            ...
            int bpp = (fmt == FMT_LUMINANCE) ? 1 : (fmt == FMT_RGB16 || fmt == FMT_RGBA16) ? 2 : 4;
#ifdef __RETROGO__
            memory = (uint8*)rg_alloc(width * height * bpp, MEM_SLOW);
#else
            memory = new uint8[width * height * bpp];
#endif
            if (memory) {
                if (data) {
                    memcpy(memory, data, width * height * bpp);
                } else {
                    memset(memory, 0, width * height * bpp);
                }
            }
        }
...
// Mesh
    struct Mesh {
        ...
        void init(Index *indices, int iCount, ::Vertex *vertices, int vCount, int aCount) {
            ...
            if (indices && vertices) {
                ...
                iBuffer = (Index*)OL_ALLOC_NP(iCount * sizeof(Index));
                vBuffer = (Vertex*)OL_ALLOC_NP(vCount * sizeof(Vertex));
    ...
    void init() {
        LOG("Renderer : %s\n", "Software");
        LOG("Version  : %s\n", "0.1");
        swViewport = short4(0, 0, 320, 240);
        swClipRect = short4(0, 0, 320, 240);
        #ifdef __RETROGO__
    ...
    void resize() {
        if (swDepth) rg_free(swDepth);
        swDepth = (DepthSW*)rg_alloc(Core::width * Core::height * sizeof(DepthSW), MEM_SLOW);
        if (swDepth) memset(swDepth, 0xFF, Core::width * Core::height * sizeof(DepthSW));
        swClipRect = short4(0, 0, Core::width, Core::height);
    }
    ...
    void setScissor(const short4 &s) {
        swClipRect.x = s.x;
        swClipRect.y = swViewport.w - (s.y + s.w);
        swClipRect.z = s.x + s.z;
        swClipRect.w = swViewport.w - s.y;
    }

    void setDepthTest(bool enable) { swDepthTest = enable; }

    void setDepthWrite(bool enable) { swDepthWrite = enable; }
            ...
            if (!swDepthTest || swDepth[x] >= z) {
                ...
                if (curTile) {
                    ...
                    if (index != 0) {
                        ...
                        if (swDepthWrite) swDepth[x] = z;
                    }
                } else if (Core::active.textures[0] && Core::active.textures[0]->memory) {
                    Texture *tex = (Texture*)Core::active.textures[0];
                    uint32 bgWidth = tex->width;
                    uint32 bgHeight = tex->height;

                    if (bgWidth > 0 && bgHeight > 0) {
                        int tx = (u * bgWidth) >> 15;
                        int ty = (v * bgHeight) >> 15;

                        if (tx < 0) tx = 0; else if (tx >= (int)bgWidth) tx = bgWidth - 1;
                        if (ty < 0) ty = 0; else if (ty >= (int)bgHeight) ty = bgHeight - 1;

                        if (tex->fmt == FMT_RGB16 || tex->fmt == FMT_RGBA16) {
                            swColor[x] = ((uint16*)tex->memory)[ty * bgWidth + tx];
                            if (swDepthWrite) swDepth[x] = z;
                        } else if (tex->fmt == FMT_RGBA) {
                            uint32 color32 = ((uint32*)tex->memory)[ty * bgWidth + tx];
                            uint8 r = (color32 >> 0) & 0xFF;
                            uint8 g = (color32 >> 8) & 0xFF;
                            uint8 b = (color32 >> 16) & 0xFF;
                            swColor[x] = CONV_COLOR(r, g, b);
                            if (swDepthWrite) swDepth[x] = z;
                        }
                    }
                }
    ...
    void drawTriangle(Index *indices) {
        ...
        if (b->y < swClipRect.y || t->y > swClipRect.w || b->y == t->y)
            return;
